    ...
    <div class="container">
      <main>
        <div class="py-5 text-center">
          ...
          $menu =  $row['menu'];
          $conn->close();
          echo " <img src='data:image/jpeg;base64," . base64_encode($menu) . "' ' style=' width: 50%;height: 100%;' />";
          echo'
        </div>
        <div>
          <div>
            <form class="needs-validation" enctype="multipart/form-data"  method="post" action="updateMenu.php" >
              <input hidden value= '.$id.' name="id">
              <label for="menu" class="form-label">Update menu.</label><span class="text-muted">( as 400 x * png )</span>
              <div class="input-group mb-3">
                <input type="file" class="form-control" name="menu" id="menu"required>
                <label class="input-group-text" for="inputGroupFile02">Upload</label>
              </div>

              <button class="w-25 btn btn-primary btn-lg" type="submit">Update Menu</button>
            </form>
          </div>
        </div>
        '?>
